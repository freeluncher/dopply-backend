...
class FetalMonitoringService:l Monitoring Classification dan Business Logic
from typing import List, Dict, Any, Optional
from datetime import datetime
import statistics
import json
from app.models.medical import FetalClassification, R        # Create monitoring record using correct field names
        from app.models.medical import RecordSource
        
        record = Record(
            patient_id=request.patient_id,
            doctor_id=user_id,
            source=RecordSource.clinic,  # Default to clinic monitoring
            start_time=request.start_time,
            end_time=request.end_time or get_local_now(),
            bpm_data=bmp_data if bmp_data else [],  # Use bpm_data field, not heart_rate_data
...
class FetalMonitoringService:
    ...
    def classify_fetal_heart_rate(
        ...
        Args:
            fhr_data: List of fetal heart rate values
            gestational_age: Gestational age in weeks
            maternal_age: Maternal age in years
            duration_minutes: Duration of monitoring in minutes
        ...
        if not 15 <= maternal_age <= 50:
            raise ValueError("Maternal age must be between 15 and 50 years")
        
        # Use existing classify_fetal_bmp method
        result = FetalMonitoringService.classify_fetal_bpm(fhr_data, gestational_age)
        
        # Add additional fields for enhanced response
        result.update({
            "maternal_age": maternal_age,
            "duration_minutes": duration_minutes or len(fhr_data) * 2,  # Estimate 2 minutes per reading
            "analysis_timestamp": datetime.now().isoformat(),
            "confidence_score": 0.85,  # Static confidence for now
        })
